...
public class FarmerControllerV5 {

    private final FarmerServiceV5 farmerService;

    public FarmerControllerV5(FarmerServiceV5 serviceV5) { this.farmerService = serviceV5; }
    ...
    @PostMapping("/farmer")
    public ResponseEntity<FarmerV5> createFarmer(@RequestBody FarmerV5 farmer) {
        try {
    ...
    public ResponseEntity<FarmerV5> getFarmerById(@PathVariable Long id) {
    ...
    public ResponseEntity<FarmerV5> updateFarmerById(@PathVariable Long id, @RequestBody FarmerV5 updatedFarmer) {
    ...
    @GetMapping("/farmer")
    public ResponseEntity<FarmerV5> getFarmerBySurname(@RequestParam String surname) {
        var farmer = farmerService.findFarmerBySurname(surname);
        return farmer.map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
    }

    @GetMapping("/farmers")
    public ResponseEntity<List<FarmerV5>> getFarmersWithinIdIntervall(@RequestParam(required = false) Long from, @RequestParam(required = false) Long to) {
